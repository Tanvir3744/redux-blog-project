...
const Blog = () => {
  ...
  console.log(blogs.filter(blog => blog?.blogTitle?.toLowerCase().includes(input)))
      ...
      <div className="relative  pt-8 pb-20 px-4 sm:px-6 lg:pt-16 lg:pb-16 lg:px-8">
        <div className=' max-w-7xl mx-auto'>
          ...
          <div className='mt-12 max-w-lg mx-auto grid gap-5 lg:grid-cols-3 lg:max-w-none'>
            {
              blogs.filter(blog=> blog.blogTitle.toLowerCase().includes(input)).map(blog => <Card key={blog.id} blog={blog} />)
            }
          </div>
        </div>
      </div>
